...
const BEACON_HEADER = [0x5a, 0x52, 0x45, BEACON_VERSION];
...
function createDataGramBuffer(identity, mailbox) {
  ...
  return Buffer.concat([Buffer.from(BEACON_HEADER), identity, dgMailbox]);
}
...
function readDataGramBuffer(dgBuffer) {
  try {
    ...
    if ((dgBuffer.compare(Buffer.from(BEACON_HEADER), 0, 4, 0, 4) !== 0)) throw Error;
...
class ZBeacon {
   ...
   * @param {string} options.broadcast - Broadcast address
   ...
   */
  constructor({ identity, mailbox, broadcast, port = 5670, interval = 1000, zyrePeers }) {
