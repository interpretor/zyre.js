...
function readDataGramBuffer(dgBuffer) {
  if (
    !dgBuffer.length === 22 &&
    !(dgBuffer.compare(Buffer.from(DG_HEADER), 0, 4, 0, 4) === 0)
  ) {
    return undefined;
  }
...
module.exports = class ZBeacon {
  ...
  /**
   * ZBeacon implements the discovery beacon defined in the ZRE protocol, it
   * listens for foreign beacons and broadcasts his own datagram
   *
   * @constructor
   * @param {Buffer} identity
   * @param {number} mailbox
   ...
   */
  constructor({ identity, mailbox, ifaceData, zyrePeers }) {
      ...
      this._nodeSock.bind(() => {
        ...
        this._broadcastTimer = setInterval(() => {
          this._nodeSock.send(this._dgBuffer, BEACON_PORT, this._bcAddr, () => {
            debug(`to ${this._bcAddr}:${BEACON_PORT}`);
          });
        }, BEACON_INTERVAL);
        resolve();
      });
  ...
  /**
   * Starts listening for foreign beacons and pushes discovered peers to the
   * ZyrePeers object
   *
   * @return {Promise}
   */
