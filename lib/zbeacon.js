...
module.exports = class ZBeacon extends EventEmitter {

  constructor(id, mailbox, ifaceData) {
    super();

    this.bcAddress = ifaceData.broadcast;
    this.nodeSock = initializeNodeSock();
    this.peerSock = initializePeerSock();
    this.dataGramBuffer = createDataGramBuffer(id, mailbox);
  }
    ...
    return new Promise((resolve) => {
      this.nodeSock.bind(() => {
        ...
        this.broadcasting = setInterval(() => {
          this.nodeSock.send(this.dataGramBuffer, BEACON_PORT, this.bcAddress, () => {
            debug(`sent beacon to ${this.bcAddress}:${BEACON_PORT} ` +
              `[${this.dataGramBuffer.toString('hex')}]`);
          });
        }, BEACON_INTERVAL);
  ...
  startListening() {
    this.peerSock.on('listening', () => {
      ...
      this.emit('listening', this.peerSock.address());
    });

    this.peerSock.on('message', (msg, rinfo) => {
      // Return if received own beacon
      if (msg.equals(this.dataGramBuffer)) {
        return;
      }
      ...
      this.emit('beacon', dataGram, rinfo);
    });
