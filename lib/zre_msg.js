...
function putStrings(strArr) {
  ...
  strArr.forEach((e) => {
    stringsBuffer = Buffer.concat([stringsBuffer, putLongString(String(e))]);
  });
...
function putDictionary(obj) {
  ...
  Object.keys(obj).forEach((i) => {
    dictBuffer = Buffer.concat([dictBuffer, putString(String(i)), putLongString(String(obj[i]))]);
  });
  return dictBuffer;
}
...
class ZreMsg {
  ...
  static read(buffer, frame) {
    try {
      if ((buffer.compare(Buffer.from(ZRE_HEADER), 0, 2, 0, 2) !== 0)) throw Error;
