...
class ZyrePeers extends EventEmitter {
  ...
  push({ identity, sequence, address, mailbox, endpoint, status, name, headers }) {
    // If received a message with mailbox === 0, disconnect from that peer (zre beacon protocol)
    if (mailbox === 0) {
      if (this._peers[identity]) {
         ...
         */
        this.emit('disconnect', this._peers[identity]);
      }
      this.disconnect(identity);
      return undefined;
    }
   ...
   */
  disconnect(identity) {
    const peer = this._peers[identity];
    if (peer) {
