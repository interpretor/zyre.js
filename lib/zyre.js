...
class Zyre extends EventEmitter {
  ...
  /**
   * Finds a free TCP port on the host, starts the ZyreNode and the ZBeacon, adds listeners and
  ...
  start(callback) {
    ...
    this._zyrePeers.on('evasive', this._evasiveHandler);
    this._zyrePeers.on('expired', this._expiredHandler);
    this._zyrePeers.on('back', this._backHandler);
  ...
  _createHandler() {
    ...
    this._backHandler = (peer) => {
      /**
       * @event Zyre#back
       * @property {string} identity - Identity of the peer
       * @property {string} name - Name of the peer
       */
      this.emit('back', peer.getIdentity(), peer.getName());
    };
    ...
    this._connectHandler = (peer) => {
       ...
       * @property {string} name - Name of the peer
       */
      this.emit('connect', peer.getIdentity(), peer.getName());
    };
