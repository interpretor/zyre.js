...
class Zyre extends EventEmitter {
   ...
   */
  constructor({ name, iface, headers } = {}) {
    super();
  ...
  start(callback) {
    ...
    this._zyrePeers.on('disconnect', (peer) => {
      /**
    ...
    this._zyrePeers.on('evasive', (peer) => {
      /**
       * @event Zyre#evasive
       * @property {string} identity - Identity of the peer
       * @property {string} name - Name of the peer
       */
      this.emit('evasive', peer.getIdentity(), peer.getName());
    });

    this._zyrePeers.on('expired', (peer) => {
      /**
       * @event Zyre#expired
       * @property {string} identity - Identity of the peer
       * @property {string} name - Name of the peer
    ...
    return new Promise((resolve) => {
      ZHelper.getFreePort(this._ifaceData.address, MAILBOX).then((port) => {
        ...
        this._zyreNode.on('connect', (identity, name) => {
          /**
           * @event Zyre#connect
           * @property {string} identity - Identity of the peer
           * @property {string} name - Name of the peer
           */
          this.emit('connect', identity, name);
        });

        this._zyreNode.on('message', (identity, name, message, group) => {
          /**
           * @event Zyre#message
           * @property {string} identity - Identity of the peer
           * @property {string} name - Name of the peer
           ...
           */
          this.emit('message', identity, name, message, group);
        });
  ...
  /**
   * Stops listening, closes all sockets and disconnects from all peers
   *
   * @param {function} callback - Executed on success
   * @return {Promise}
   */
   ...
   * @return {Zyre}
   */
  static new({ name, iface, headers } = {}) {
    return new Zyre({ name, iface, headers });
  }
}
