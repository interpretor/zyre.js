...
class ZyrePeer extends EventEmitter {
  ...
  connect() {
    ...
    debug(`connected to ${this._identity}`);
  }
  ...
  disconnect() {
    ...
    debug(`disconnected from ${this._identity}`);
    /**
  ...
  send(msg) {
    if (this._connected) {
      ...
      msg.send(this._socket).then((cmd) => {
        debug(`sent message (${cmd}) to ${this._identity}, seq ${this._sequenceOut}`);
      });
    }
  }
  ...
  _setSequence(sequence) {
    if (typeof sequence !== 'undefined') {
      ...
      if (sequence !== expectedSeq) {
        debug(`${this._identity} sent wrong sequence (${sequence}), expected ${expectedSeq}`);
        this.disconnect();
        return false;
      }
  ...
  _setAddress(address, mailbox) {
    if (typeof address !== 'undefined' && typeof mailbox !== 'undefined') {
      ...
      if (mailbox === 0) {
        debug(`received disconnect beacon from ${this._identity}`);
        this.disconnect();
        return false;
      }
  ...
  _createHandler() {
    ...
    this._evasiveHandler = () => {
      ...
      debug(`${this._identity} is evasive at ${this._evasiveAt}`);
    ...
    this._expiredHandler = () => {
      debug(`${this._identity} is expired at ${Date.now()}`);
      /**
