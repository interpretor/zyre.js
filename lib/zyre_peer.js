...
class ZyrePeer extends EventEmitter {
  ...
  update({ sequence, address, mailbox, endpoint, status, name, headers }) {
    if (typeof sequence !== 'undefined') {
      ...
      // Close on wrong sequence number
    ...
    if (typeof address !== 'undefined' && typeof mailbox !== 'undefined') {
      // If received a message with mailbox set to 0, close the peer (zre beacon protocol)
      if (mailbox === 0) {
   ...
   */
  _setTimeouts() {
    // Reset the evasive and expired status when restarting the timeouts
    this._evasiveAt = 0;
  ...
  _createHandler() {
    ...
    /**
     * Reconnect when the peer is available again
     *
     * @protected
     * @fires ZyrePeer#back
     */
